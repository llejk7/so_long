...
/*   Updated: 2024/12/28 19:18:37 by kjell            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void update_player_position(t_game *game, int new_x, int new_y)
{
    ...
    ft_putnbr_fd(++game->steps, 1); // Increment step count
    ft_putstr_fd("\n", 1);
}
...
void move_player(t_game *game, int dx, int dy)
{
    ...
    if (new_x < 0 || new_y < 0 || new_x >= game->map_width
        ...
        return; // Wall or invalid move
    handle_collectible(game, new_x, new_y);
    if (game->map[new_y][new_x] == 'E')
    {
        handle_exit(game); // Exit logic
        return; // Prevent replacing the exit with the player
    }
